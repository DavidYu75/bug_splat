...
let sayComment = (ratsLeft) => {
  ...
  if (ratsLeft >= 8) {
    ...
    document.getElementById(
      "comment"
    ).innerHTML = `Comment: ${defiantComments[randomDefiantComment]}`;
    ...
    document.getElementById(
      "comment"
    ).innerHTML = `Comment: ${mercyComments[randomMercyComment]}`;
...
let splat = (rat) => {
  if (splattedRats.includes(rat) != true) {
    ...
    let splatNoise = new Audio("images/splatSound.mp3");
    splatNoise.play();
  }
};
...
function reset() {
  ...
  for (let i = 0; i < imgElements.length; i++) {
    imgElements[i].src = "images/rattata.png";
  }
